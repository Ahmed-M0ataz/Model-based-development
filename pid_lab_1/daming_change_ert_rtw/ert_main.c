...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(daming_change_M, 0);
  ...
  daming_change_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(daming_change_M->extModeInfo, &rtmGetTFinal
    (daming_change_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(daming_change_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(daming_change_M);
